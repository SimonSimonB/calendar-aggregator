...
impl DateExtractor for DateWithGermanMonthExtractor {
    fn get_regexp(&self) -> regex::Regex {
        ...
        Regex::new(&format!(
            r"({})\.?\s?({})",
            DAY_OF_MONTH, month_names_joined
        ))
        .unwrap()
    }
...
pub fn extract_datetimes(div: &ElementRef) -> Vec<DateMatch> {
    ...
    let date_extractors: Vec<&dyn DateExtractor> = vec![
        &DateWithNumericMonthExtractor {},
        &DateWithGermanMonthExtractor {},
    ];
    // println!("{}\n", text);
    for date_extractor in date_extractors {
        for captured in date_extractor
            .get_regexp()
            .find_iter(&text)
            .zip(date_extractor.get_regexp().captures_iter(&text))
        {
            match date_extractor.extract(captured.1) {
                None => (),
                Some(date) => results.push(DateMatch {
                    date: date,
                    matched_string: captured.0.as_str().to_owned(),
                }),
            }
        }
        if results.len() > 0 {
